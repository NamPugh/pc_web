...
export default function AccountPage() {
  ...
  if (!ready) {
    return <div className="rounded-lg border border-slate-200 bg-white p-8 text-center text-slate-500 shadow-sm">Đang tải tài khoản...</div>;
  }
    ...
    return (
      <section className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[#d71920]">Tài khoản</p>
          <h1 className="mt-2 text-4xl font-bold tracking-tight text-slate-950">Đăng nhập để quản lý trải nghiệm mua hàng</h1>
        ...
        <div className="rounded-lg bg-[#111111] p-6 text-white shadow-sm">
          <ShieldCheck className="size-8 text-red-100" />
    ...
    <section className="space-y-6">
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-0 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="bg-[#111111] p-6 text-white sm:p-8">
            <div className="grid size-20 place-items-center rounded-lg bg-white text-[#d71920] shadow-lg shadow-black/15">
          ...
          <div className="grid gap-4 p-6 sm:p-8">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                <Mail className="size-5 text-[#d71920]" />
                <p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Email</p>
                <p className="mt-1 truncate font-semibold text-slate-950">{user.email}</p>
              </div>
              <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                <ShieldCheck className="size-5 text-[#d71920]" />
                <p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Vai trò</p>
                <p className="mt-1 font-semibold text-slate-950">{user.role === "admin" ? "Quản trị viên" : "Khách hàng"}</p>
              </div>
              <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                <Phone className="size-5 text-[#d71920]" />
                <p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Số điện thoại</p>
                <p className="mt-1 font-semibold text-slate-950">{user.phone || "Chưa cập nhật"}</p>
              </div>
              <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                <MapPin className="size-5 text-[#d71920]" />
                <p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Địa chỉ</p>
                <p className="mt-1 line-clamp-1 font-semibold text-slate-950">{user.address || "Chưa cập nhật"}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {quickActions.map((item) => {
          ...
          return (
            <Link key={item.href} to={item.href} className="group rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-red-200 hover:shadow-sm">
              ...
              <h2 className="mt-4 text-xl font-bold tracking-tight text-slate-950">{item.label}</h2>
